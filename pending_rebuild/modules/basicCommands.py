...
class basicCommands:
    ...
    def __init__(self):
        ...
        if basicCommands.instCount:
            msg = (
                "There is already an instance of this module running... "
    ...
    def commandCheck(
        self, guild: str, channel: str, roles: list, user: str, message: str
    ) -> dict:
        """
        ...
        try:
            ...
            # Role restrictions
            if len(cData["roles"]) and not (
                set(roles).intersection(set(cData["roles"]))
            ):  # noqa
    ...
    def modCommand(self, guild: str, msg: str) -> dict:
        ...
        if modtype == "append":
            ...
            if tartype == "int":
                ...
                self.bcConfig["guilds"][guild]["commands"][cname][mod] += int(
                    text
                )  # noqa
            if tartype == "str":
        ...
        if modtype == "remove":
            ...
            if (
                text
                not in self.bcConfig["guilds"][guild]["commands"][cname][mod]
            ):  # noqa
                return {
                    "status": False,
                    "response": f'"{text}" was not found in "{mod}".',
                }
            ind = self.bcConfig["guilds"][guild]["commands"][cname][mod].index(
                text
            )  # noqa
    ...
    def commandList(
        ...
        """
        logger.debug(
            f"commandList Start: {guild}, {channel}, {roles}, "
            f"{user}, {message}"
        )
        self.checkGuild(guild)
        ...
        if self.bcConfig["guilds"][guild]["listcontrol"]:
            ...
            if allowedroles and not (
                any(role in allowedroles for role in roles)
            ):  # noqa
    ...
    def commandHelp(self, guild: str, roles: list) -> dict:
        ...
        response = (
            "Basic Commands Module command list: ```"
            "+ command!list (search)\n"
    ...
    def guild_permissions(
        self, guild: str, user: str, channel: str, roles: str
    ) -> bool:
        ...
        if not (
            set(roles).intersection(
                set(self.bcConfig["guilds"][guild]["allowedroles"])
            )
        ):
